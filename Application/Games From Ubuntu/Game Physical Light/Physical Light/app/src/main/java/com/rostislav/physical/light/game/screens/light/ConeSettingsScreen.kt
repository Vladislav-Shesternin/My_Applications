...
class ConeSettingsScreen(override val game: GdxGame): AdvancedUserScreen(game) {
    ...
    private val aSettings = ASettingsConeLight(this)
    ...
    private fun AdvancedStage.addSettings() {
        addActor(aSettings)
        aSettings.setBounds(172f, 113f, 800f, 854f)

        aSettings.apply {
            coneDegreeBlock     = { coneLight?.coneDegree = it }
            distanceBlock       = { coneLight?.distance = it.toB2 }
            softnessLengthBlock = { coneLight?.setSoftnessLength(it) }

            isBlurBlock = { rayHandler.setBlur(it) }
            isSoftBlock = { coneLight?.isSoft = it }
            isXrayBlock = { coneLight?.isXray = it }
            isStaticLightBlock = { coneLight?.isStaticLight = it }
        }
    }
    ...
    private fun createL_Cone() {
        coneLight = ConeLight(rayHandler, 1000, GdxColor.light.cpy().apply { a = 0f }, 700f.toB2, 1496f.toB2, 540f.toB2, 180f, ASettingsConeLight.coneDegreeValue).apply {
