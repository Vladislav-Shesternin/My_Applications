...
import com.rostislav.physical.light.game.actors.settings.ASettingsPointLight
...
class PointSettingsScreen(override val game: GdxGame): AdvancedUserScreen(game) {
    ...
    private val aSettings = ASettingsPointLight(this)
    ...
    private fun AdvancedStage.addSettings() {
        ...
        aSettings.apply {
            distanceBlock       = { pointLight?.distance = it.toB2 }
            softnessLengthBlock = { pointLight?.setSoftnessLength(it) }

            isBlurBlock = { rayHandler.setBlur(it) }
            isSoftBlock = { pointLight?.isSoft = it }
            isXrayBlock = { pointLight?.isXray = it }
            isStaticLightBlock = { pointLight?.isStaticLight = it }
        }
    }
    ...
    private fun createB_CircleUser() {
        ...
        bCircleUser.create(1434f,478f,125f,125f)

        pointLight?.attachToBody(bCircleUser.body)
    }
    ...
    private fun createJ_Motor() {
        ...
        bCircleUser.actor?.preDrawArray?.add(AdvancedGroup.Static.Drawer { alpha ->
    ...
    private fun createL_Point() {
        pointLight = PointLight(rayHandler, 1000, GdxColor.light.cpy().apply { a = 0f }, 700f.toB2, 1496f.toB2, 540f.toB2).apply {
            setDistance(ASettingsPointLight.distanceValue.toB2)
            setSoftnessLength(ASettingsPointLight.softnessLengthValue)
            isSoft = ASettingsPointLight.isSoftValue
            isXray = ASettingsPointLight.isXrayValue
            isStaticLight = ASettingsPointLight.isStaticLightValue

            rayHandler.setBlur(ASettingsPointLight.isBlurValue)
        }
    }
