...
class ChainSettingsScreen(override val game: GdxGame): AdvancedUserScreen(game) {
    ...
    private val aSettings = ASettingsChainLight(this)
    ...
    override fun render(delta: Float) {
        ...
        if (stageUI.root.color.a != 1f) chainLight?.setColor(chainLight?.color?.apply { a = stageUI.root.color.a })
    }
    ...
    private fun AdvancedStage.addSettings() {
        ...
        aSettings.apply {
            distanceBlock       = { chainLight?.distance = it.toB2 }
            softnessLengthBlock = { chainLight?.setSoftnessLength(it) }

            isBlurBlock = { rayHandler.setBlur(it) }
            isSoftBlock = { chainLight?.isSoft = it }
            isXrayBlock = { chainLight?.isXray = it }
            isStaticLightBlock = { chainLight?.isStaticLight = it }
        }
    }
    ...
    private fun createB_RectUser() {
        ...
        chainLight?.attachToBody(bRectUser.body)
    }
    ...
    private fun createL_Chain() {
        val chainArr = arrayOf(
            62.5, -62.5,   62.5, 62.5,
        ).map { it.toFloat().toB2 }.toFloatArray()
        chainLight = ChainLight(rayHandler, 1000, GdxColor.light.cpy().apply { a = 0f }, 400f.toB2, -1, chainArr).apply {
