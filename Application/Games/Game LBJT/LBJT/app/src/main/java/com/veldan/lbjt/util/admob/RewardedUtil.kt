...
class RewardedUtil(private val activity : MainActivity) {

    private val adRequest by lazy { AdRequest.Builder().build() }
    private var rewardedAd: RewardedAd? = null

    fun load(successBlock: () -> Unit, failedBlock: () -> Unit) {
        RewardedAd.load(activity, activity.getString(R.string.reward_UID), adRequest, object : RewardedAdLoadCallback() {
            override fun onAdLoaded(ad: RewardedAd) {
                log("RewardedUtil: loaded.")
                rewardedAd = ad.apply { fullScreenContentCallback = object: FullScreenContentCallback() {
