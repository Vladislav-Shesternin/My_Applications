...
import com.veldan.lbjt.game.utils.NICKNAME_LENGTH
...
class BGDialogNickname(override val screenBox2d: AdvancedBox2dScreen): AbstractBodyGroup() {
    ...
    private fun AdvancedStage.addIndicator() {
        addActor(aIndicator)
        aIndicator.setBoundsStandart(320f, 870f, 60f, 60f)
    }
    ...
    private fun upDialogNickname() {
        bStatic.body?.apply {
            runGDX {
                setTransform(position.add(Vector2(0f, 207f).toStandart.toB2), 0f)
