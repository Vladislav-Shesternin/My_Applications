...
class BGPrismaticJoint_Diagonal(override val screenBox2d: AdvancedBox2dScreen): AbstractBodyGroup() {
    ...
    private fun initB_Dynamic() {
        arrayOf(bDynamicCircle).onEach { it.apply {
            ...
            update(screenBox2d.game.assets.C_DYNAMIC_DIAGONAL)
        } }
    }
    ...
    private fun createJ_Prismatic() {
        createJoint(jPrismatic, PrismaticJointDef().apply {
            ...
            val radian = -45f * DEGTORAD
            localAxisA.set(cos(radian), sin(radian))
        })
    }
