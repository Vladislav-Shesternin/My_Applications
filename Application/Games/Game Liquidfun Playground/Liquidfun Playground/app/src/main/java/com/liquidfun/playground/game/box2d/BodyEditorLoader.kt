...
class BodyEditorLoader {
    ...
    private val tmpShapeFixtureDef = FixtureDef()

    fun getShape(name: String, scale: Float = 1f): Shape {
        val rbModel = internalModel.rigidBodies[name] ?: throw RuntimeException("Name '$name' was not found.")
        val origin = Vector2().set(rbModel.origin).scl(scale)
        run {
            var i = 0
            ...
            while (i < n) {
                ...
                polygonShape.set(vertices)
                tmpShapeFixtureDef.shape = polygonShape
                //body.createFixture(fd)
                var ii = 0
                val nn = vertices.size
                while (ii < nn) {
        ...
        return tmpShapeFixtureDef.shape
    }
