...
import nsl.radead.egyptlegacy.game.actors.masks.Mask
import nsl.radead.egyptlegacy.game.actors.slots.FillStrategy
...
import nsl.radead.egyptlegacy.game.utils.Layout.SlotGroup as LSG
...
class SlotGroup(override val screen: AdvancedScreen): AdvancedGroup() {
    ...
    companion object {
        ...
        const val TIME_BETWEEN_SHOW_WIN    = 0.4f
        const val TIME_WAIT_AFTER_SHOW_WIN = 0.4f
    }

    private val mask       = Mask(screen, alphaHeight = 340)
    ...
    // Logic
    // ------------------------------------------------------------------------
    suspend fun spin() = CompletableDeferred<Unit>().also { continuation ->
        spinCounter++
