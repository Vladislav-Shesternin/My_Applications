package nsl.radead.egyptlegacy.util
...
abstract class AbstractDataStore {
    ...
    abstract inner class DataStoreElement<T> {
        ...
        open suspend fun collect(block: suspend (T?) -> Unit) {
            nsl.radead.egyptlegacy.appContext.dataStore.data.collect { block(it[key]) }
        }

        open suspend fun update(block: suspend (T?) -> T) {
            nsl.radead.egyptlegacy.appContext.dataStore.edit { it[key] = block(it[key]) }
        }

        open suspend fun get(): T? {
            return nsl.radead.egyptlegacy.appContext.dataStore.data.first()[key]
        }
    }
}