package nsl.radead.egyptlegacy.game.actors.button
...
open class AButton(
    ...
    private fun getStyleByType(type: Type) = when(type) {
        Type.DEFAULT -> AButtonStyle(
            default  = spriteUtil.BTN_DEF,
            pressed  = spriteUtil.BTN_PRS,
            disabled = spriteUtil.BTN_PRS,
        )
        Type.GO -> AButtonStyle(
            default  = spriteUtil.GO,
            pressed  = spriteUtil.GO_PRESS,
            disabled = spriteUtil.GO_PRESS,
        )
        Type.MENU -> AButtonStyle(
            default  = spriteUtil.MENU_DEF,
            pressed  = spriteUtil.MENU_PRS,
            disabled = spriteUtil.MENU_PRS,
        )
    }
