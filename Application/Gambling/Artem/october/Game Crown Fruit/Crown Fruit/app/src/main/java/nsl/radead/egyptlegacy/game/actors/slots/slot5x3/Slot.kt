...
import nsl.radead.egyptlegacy.game.utils.Layout
...
import nsl.radead.egyptlegacy.game.utils.Layout.Slot as LS

class Slot(override val screen: AdvancedScreen): AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 3
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 34

        const val TIME_SPIN_SLOT = 5f
    }
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        runGDX {
            addAction(Actions.sequence(
                Actions.moveTo(x, LS.endY , TIME_SPIN_SLOT, Interpolation.fade),
                Actions.run {
                    reset()
