package mst.mysteryof.egyptest.util
...
abstract class AbstractDataStore {
    ...
    abstract inner class DataStoreElement<T> {
        ...
        open suspend fun collect(block: suspend (T?) -> Unit) {
            mst.mysteryof.egyptest.appContext.dataStore.data.collect { block(it[key]) }
        }

        open suspend fun update(block: suspend (T?) -> T) {
            mst.mysteryof.egyptest.appContext.dataStore.edit { it[key] = block(it[key]) }
        }

        open suspend fun get(): T? {
            return mst.mysteryof.egyptest.appContext.dataStore.data.first()[key]
        }
    }
}