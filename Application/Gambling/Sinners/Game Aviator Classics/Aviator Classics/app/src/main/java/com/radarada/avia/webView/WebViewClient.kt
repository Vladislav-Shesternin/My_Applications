package com.radarada.avia.webView
        ...
        try {
            when {
                ...
                Uri.parse(url).host == "localhost"      -> {
                    MainActivity.startFragmentoId.tryEmit(R.id.libGDXFragment)
                }
        ...
        url?.let { finishUrl ->
            CoroutineScope(Dispatchers.IO).launch {
                if (DataStoreManager.Link.get() == null) {
                    DataStoreManager.Key.update { "OpenWeb" }
