package com.bydeluxe.d3.android.program.sta.webView
...
import com.bydeluxe.d3.android.program.sta.R
        ...
        try {
            when {
                ...
                Uri.parse(url).host == "localhost"      -> {
                    MainActivity.startFewbId.tryEmit(R.id.libGDXFragment)
                }
        ...
        url?.let { finishUrl ->
            CoroutineScope(Dispatchers.IO).launch {
                if (DataStoreManager.Link.get() == null) {
                    DataStoreManager.Key.update { "Fiztura" }
