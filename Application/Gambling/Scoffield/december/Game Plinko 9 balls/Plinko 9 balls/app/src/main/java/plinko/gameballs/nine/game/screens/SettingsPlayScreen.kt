...
class SettingsPlayScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun AdvancedStage.addRulles() {
        val settings = Image(game.gameAssets.SETTINGS)
        addActor(settings)
        settings.setBounds(93f, 141f, 951f, 1521f)
    }
    ...
    private fun AdvancedStage.addSound() {
        val sound = ACheckBox(this@SettingsPlayScreen, ACheckBox.Static.Type.SOUND)
        addActor(sound)
    ...
    private fun AdvancedStage.addCB() {
        val sound = ACheckBox(this@SettingsPlayScreen, ACheckBox.Static.Type.DEFAULT)
        val music = ACheckBox(this@SettingsPlayScreen, ACheckBox.Static.Type.DEFAULT)

        addActors(sound, music)

        sound.apply {
            if (game.soundUtil.isPause.not()) check(false)
            setBounds(515f, 981f, 307f, 97f)
            setOnCheckListener { game.soundUtil.isPause = it.not() }
        }
        music.apply {
            if (game.musicUtil.music!!.isPlaying) check(false)
            setBounds(515f, 727f, 307f, 97f)
            setOnCheckListener { if (it) game.musicUtil.music?.play() else game.musicUtil.music?.pause() }
        }
    }
