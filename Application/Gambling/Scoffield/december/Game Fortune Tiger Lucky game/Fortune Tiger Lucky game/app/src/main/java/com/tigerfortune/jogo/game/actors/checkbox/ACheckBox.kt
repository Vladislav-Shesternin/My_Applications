...
import com.tigerfortune.jogo.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            screen.game.soundUtil.apply { play(mouse_click) }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.MUSIC -> Static.ACheckBoxStyle(
            default = assets.music_def,
            checked = assets.music_check,
        )
        Static.Type.PAUSE -> Static.ACheckBoxStyle(
            default = assets.pause_def,
            checked = assets.pause_check,
        )
        Static.Type.CHECK_BOX -> Static.ACheckBoxStyle(
            default = assets.cb_def,
            checked = assets.cb_check,
        )
        Static.Type.VOLUME -> Static.ACheckBoxStyle(
            default = TextureEmpty.region,
            checked = assets.volume,
        )
    }
