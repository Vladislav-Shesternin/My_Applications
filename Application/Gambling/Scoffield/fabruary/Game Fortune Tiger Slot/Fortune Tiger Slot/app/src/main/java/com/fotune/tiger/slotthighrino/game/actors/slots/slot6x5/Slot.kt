...
import com.fotune.tiger.slotthighrino.game.utils.Layout
...
import com.fotune.tiger.slotthighrino.game.utils.Layout.Slot as LS

class Slot(override val screen: AdvancedScreen): AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 3
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 6

        const val TIME_SPIN_SLOT = 0.8f
    }
    ...
    private fun addSlotItemList() {
        ...
        (fakeSlotItemImageList.reversed() + intermediateSlotItemImageList + winSlotItemImageList.reversed()).onEach { slotItemImage ->
            ...
            slotItemImage.apply {
                drawable = TextureRegionDrawable(screen.game.allAssets.listTiger.random())
                with(LS.slot) {
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        runGDX {
            addAction(Actions.sequence(
                Actions.moveTo(x, LS.endY , TIME_SPIN_SLOT, Interpolation.smoother),
                Actions.run {
                    screen.game.soundUtil.apply { play(end) }
                    reset()
