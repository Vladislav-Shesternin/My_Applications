    ...
    private fun playSound(bodyA: AbstractBody, bodyB: AbstractBody) {
        ...
        if (tmpArray.all { it.fixtureDef.isSensor.not() }) {
            when {
                ...
                -> if (currentTimeMinus(timeContactBorders) >= 500) {
                    soundUtil.apply { play(tennis_border) }
...
//                }
                ...
                -> if (currentTimeMinus(timeContactDynamic) >= 500) {
                    soundUtil.apply { play(tennis_ball_hit, /*volumeLevel * 0.2f*/) }
