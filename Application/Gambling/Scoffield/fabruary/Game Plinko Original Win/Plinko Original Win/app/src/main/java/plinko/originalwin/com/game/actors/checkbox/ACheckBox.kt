package plinko.originalwin.com.game.actors.checkbox
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            screen.game.soundUtil.apply { play(sound_button) }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.SOUND -> Static.ACheckBoxStyle(
            default = assets.button_sound_on,
            checked = assets.button_sound_off,
        )
    }
