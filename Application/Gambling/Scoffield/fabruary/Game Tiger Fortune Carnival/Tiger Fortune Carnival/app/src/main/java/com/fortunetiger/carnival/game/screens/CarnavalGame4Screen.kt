...
class CarnavalGame4Screen(override val game: LibGDXGame) : AdvancedBox2dScreen(WorldUtil()) {

    companion object {
        const val COUNT_SHAR = 15
    }
    ...
    private val aItemPanelList = List(4) { ASharPanel(this) }
    private val aResult        = AResultGroup(this).apply { color.a = 0f }
    private val aPnl           = Image(assets.pnl)

    // Field
    private val randomIndexList  = (0..assets.shars.lastIndex).shuffled()
    ...
    private fun AdvancedStage.addBack() {
        val menu = AButton(this@CarnavalGame4Screen, AButton.Static.Type.HOME)
        addActor(menu)
    ...
    private fun AdvancedStage.addPause() {
        val pause = ACheckBox(this@CarnavalGame4Screen, ACheckBox.Static.Type.PAUSE)
        addActor(pause)
    ...
    private fun AdvancedStage.addItemPanelList() {
        val panel = Image(game.allAssets.i4)
        addActor(panel)
        panel.setBounds(190f, 1574f, 766f, 87f)

        var nx = 218f

        val randomCountList = List<Int>(4) { (4..6).random() }

        aItemPanelList.onEachIndexed { index, aItemPanel ->
            addActor(aItemPanel)
            aItemPanel.setBounds(nx, 1587f, 120f, 60f)
            nx += 84f + 120f
    ...
    private fun createB_Shars() {
        repeat(COUNT_SHAR) {
            BShar(this).also { bItem ->
                ...
                val itemId4 = BodyId.Game.items[randomIndexList[3]]
