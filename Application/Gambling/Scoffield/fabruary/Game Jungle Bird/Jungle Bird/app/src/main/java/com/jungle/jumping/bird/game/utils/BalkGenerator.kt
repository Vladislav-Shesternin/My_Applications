...
import com.jungle.jumping.bird.game.box2d.bodiesGroup.BG_Balk
...
import com.jungle.jumping.bird.game.utils.Layout.BalkGenerator as LBG
       ...
       renderList.onEachIndexed { index, balk ->
           ...
           if (balk.position.x <= -LBG.balkSize.width) {
               if (renderList.size == listBalk.size) {
                   if (index == 0) {
                       if (renderList.last().position.x.toInt() in 0..980) generateStartPosition().also { pos -> balk.changePosition(pos.x, pos.y) }
                   } else {
                       if (renderList[index - 1].position.x.toInt() in 0..980) generateStartPosition().also { pos -> balk.changePosition(pos.x, pos.y) }
                   }
               }
           }
       }
