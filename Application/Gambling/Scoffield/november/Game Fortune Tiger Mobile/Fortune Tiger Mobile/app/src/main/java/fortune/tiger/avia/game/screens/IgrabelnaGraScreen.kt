...
import fortune.tiger.avia.game.actors.PanelMin
...
import fortune.tiger.avia.game.actors.image.AMovableImage
...
class IgrabelnaGraScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private val panelMini          = PanelMin(this)
    private val itemImgList        = List(3) { AImageWithId(this) }
    private val pipkaList          = List(3) { Pipka(this) }
    private val itemImgMovableList = List(3) { AMovableImage(this, winCounter, faiCounter) }
    ...
    // Field
    private val regions = game.gameAssets.ITEM_LIST.shuffled().take(3)
    private val ids     = listOf(0,1,2).shuffled()
    ...
    private fun AdvancedStage.addPanelMini() {
        addActor(panelMini)
        panelMini.apply {
            setBounds(19f, 759f, 1042f, 403f)
            addImgList()
        }
    }
    ...
    private fun AdvancedGroup.addImgList() {
        itemImgList.onEachIndexed { index, img ->
            ...
            img.setBounds(Layout.Igrushes.ids[index], Vector2(225f, 225f))
        }
    }
        ...
        coroutine?.launch {
            pipkaList.onEachIndexed { index, img ->
                runGDX {
                    ...
                    img.id      = ids[index]
                    addActor(img)
                    img.setBounds(Layout.Igrushes.pipla[index], Vector2(292f, 384f))
                }
    ...
    private fun AdvancedStage.addMovableImgList() {
        val pos = Layout.Igrushes.movable.shuffled()
        ...
        itemImgMovableList.onEachIndexed { index, img ->
            ...
            img.setBounds(pos[index], Vector2(225f, 225f))
    ...
    private fun AdvancedStage.addResult(flag: Boolean) {
        ...
        if (flag) {
            ...
            restartImg.apply {
                ...
                setOnClickListener(game.soundUtil) { stageUI.root.animHide(TIME_ANIM_SCREEN_ALPHA) { game.navigationManager.navigate(IgrabelnaGraScreen::class.java.name) } }
            }
        }
