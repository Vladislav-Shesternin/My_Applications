...
class BotlerLoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    override fun show() {
        loadSplashAssets()
        setBackBackground(game.loaderAssets.fonit.region)
        super.show()
        loadAssets()
        collectProgress()
    }
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTextureList = mutableListOf(SpriteManager.EnumTexture.fonit.data)
            loadTexture()
        }
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply {
                coff  = 0.45f
                music = WESTERN.apply { isLooping = true }
            }

            game.navigationManager.navigate(BotlerMenuScreen::class.java.name)
        }
    }
