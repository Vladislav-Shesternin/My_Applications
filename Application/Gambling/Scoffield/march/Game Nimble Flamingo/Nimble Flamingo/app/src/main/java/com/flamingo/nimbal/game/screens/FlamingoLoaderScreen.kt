...
import com.flamingo.nimbal.game.utils.region
import com.flamingo.nimbal.util.log
import kotlinx.coroutines.delay
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.launch

class FlamingoLoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply {
                coff  = 0.23f
                music = african_cinematic.apply { isLooping = true }
            }

            stageUI.root.animHide(TIME_ANIM) {
                game.navigationManager.navigate(FlamingoGameScreen::class.java.name)
            }
        }
    }
