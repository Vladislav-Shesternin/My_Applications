...
class DestroyGameScreen_2(override val game: LibGDXGame) : AdvancedUserScreen() {

    private val backImg    = Image(game.allAssets.back)
    private val bImg       = Image(game.allAssets.bb[1])
    ...
    private fun AdvancedStage.addBImg() {
        addActor(bImg)
        bImg.setBounds(482f, 150f, 223f, 378f)
    }

    private fun AdvancedStage.addDestroyImg() {
        addActor(destroyImg)
        destroyImg.setBounds(617f, 540f, 133f, 214f)
    }
    ...
    private fun createB_Builds() {
        var nx = 503f
        var ny = 464f
        game.allAssets.builds[1].split(45, 45).flatten().onEachIndexed { index, region ->
            val img = BBuild(this, region)
            img.create(nx,ny,45f,45f)
            nx += 45
            if(index.inc()%4==0) {
                nx = 503f
                ny -= 45f
