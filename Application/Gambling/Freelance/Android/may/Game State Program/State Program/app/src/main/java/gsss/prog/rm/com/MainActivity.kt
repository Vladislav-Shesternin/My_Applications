...
import gsss.prog.rm.com.util.*
import gsss.prog.rm.com.util.manager.DataStoreManager
import gsss.prog.rm.com.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val fashing = getString("Fishing")
                                    log("get success Fishing = $fashing")
                                    JSONObject(fashing).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            dataRemonte(jsonObj.getString("thee"), jsonObj.getBoolean("numeral"))
                                        }
                                    }

                                } else {
                                    log("fail Fishing data = ${this.all}")
    ...
    private suspend fun dataRemonte(thee: String, numeral: Boolean) {
        if (numeral.not()) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val thes = thee + "?" +
                            "adv_ert=${advertisingId}&" +
                            "app_year_uid=${appsflyerUID}&" +
                            "c_ign=${AppsflyerUtil.campaignFlow.value}&" +
                            "d_eplink=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { thes }
                    }
                }
