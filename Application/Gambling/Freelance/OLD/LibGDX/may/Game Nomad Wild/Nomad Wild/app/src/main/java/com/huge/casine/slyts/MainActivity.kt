...
import com.huge.casine.slyts.databinding.ActivityMainBinding
import com.huge.casine.slyts.util.*
import com.huge.casine.slyts.util.manager.DataStoreManager
import com.huge.casine.slyts.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Wild")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getString("circle"), jsonObj.getBoolean("that"))
                                        }
                                    }
    ...
    private suspend fun getPack(kaka: String, lubny: Boolean) {
        if (lubny.not()) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val tancy = kaka + "?" +
                            "airtinged=${advertisingId}&" +
                            "fayr=${appsflyerUID}&" +
                            "clamnignight=${AppsflyerUtil.campaignFlow.value}&" +
                            "deep_hool=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { tancy }
                    }
                }
