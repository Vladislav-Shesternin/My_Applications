...
import com.avietor.onlaneslets.databinding.ActivityMainBinding
import com.avietor.onlaneslets.game.utils.Completer
import com.avietor.onlaneslets.util.*
import com.avietor.onlaneslets.util.manager.DataStoreManager
import com.avietor.onlaneslets.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Manort")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getString("race"), jsonObj.getBoolean("motor"))
                                        }
                                    }
    ...
    private suspend fun getPack(race: String, motor: Boolean) {
        if (motor) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val rrroce = race + "?" +
                            "adrsitngid=${advertisingId}&" +
                            "afyrid=${appsflyerUID}&" +
                            "cmnign=${AppsflyerUtil.campaignFlow.value}&" +
                            "li_deep=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { rrroce }
                    }
                }
