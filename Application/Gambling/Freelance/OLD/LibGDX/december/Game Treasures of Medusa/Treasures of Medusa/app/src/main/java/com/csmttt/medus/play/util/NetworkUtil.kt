...
fun Activity.haveInternetConnection(): Boolean {
    ...
    (getSystemService(AppCompatActivity.CONNECTIVITY_SERVICE) as ConnectivityManager).allNetworkInfo.onEach { networkItem ->
        if (networkItem.typeName.equals("WIFI", ignoreCase = true)) haveConnectedWifi = networkItem.isConnected
        if (networkItem.typeName.equals("MOBILE", ignoreCase = true)) haveConnectedMobile = networkItem.isConnected
    }
