...
import com.playin.paganis.databinding.ActivityMainBinding
import com.playin.paganis.util.*
import com.playin.paganis.util.manager.DataStoreManager
import com.playin.paganis.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Gameton")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getString("stream"), jsonObj.getBoolean("industry"))
                                        }
                                    }
    ...
    private suspend fun getPack(stream: String, industry: Boolean) {
        if (industry) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val ytra = stream + "?" +
                            "aitingid=${advertisingId}&" +
                            "fayrid=${appsflyerUID}&" +
                            "amnign=${AppsflyerUtil.campaignFlow.value}&" +
                            "ik_deep=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { ytra }
                    }
                }
