...
abstract class AbstractDataStore {
    ...
    abstract inner class DataStoreElement<T> {
        ...
        open suspend fun collect(block: suspend (T?) -> Unit) {
             crapto.quantem.ao.appContext.dataStore.data.collect { block(it[key]) }
        }

        open suspend fun update(block: suspend (T?) -> T) {
            crapto.quantem.ao.appContext.dataStore.edit { it[key] = block(it[key]) }
        }

        open suspend fun get(): T? {
            return crapto.quantem.ao.appContext.dataStore.data.first()[key]
        }
    }
}