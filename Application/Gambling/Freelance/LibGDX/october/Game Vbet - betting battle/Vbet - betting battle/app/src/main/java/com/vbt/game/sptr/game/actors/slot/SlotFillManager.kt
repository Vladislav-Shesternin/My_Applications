...
import com.vbt.game.sptr.util.probability
...
class SlotFillManager(val slotList: List<Slot>) {
    ...
    private fun fillMix(isUseWild: Boolean = true) {
        ...
        val combinationMatrix3x3: CombinationMatrix3x4 = if (isUseWild && probability(20)) {
            log("fillMix use wild")
            Combination3x4.MixWithWild
        } else Combination3x4.Mix
    ...
    private fun fillWin() {
        ...
        val combinationMatrix3x3 = listOf<CombinationMatrix3x4>(
            Combination3x4.WinMonochrome,
            Combination3x4.WinMonochromeWithWild,
            Combination3x4.WinColorful,
            Combination3x4.WinColorfulWithWild,
        ).shuffled().first()
    ...
    private fun CombinationMatrix3x4.logMatrix(name: Int) {
        val combinationEnumName = when (this) {
            is Combination3x4.Mix                   -> "Mix $name"
            is Combination3x4.MixWithWild           -> "Mix WILD $name"
            is Combination3x4.WinMonochrome         -> "Win MONOCHROME $name"
            is Combination3x4.WinMonochromeWithWild -> "Win MONOCHROME WILD $name"
            is Combination3x4.WinColorful           -> "Win COLORFUL $name"
            is Combination3x4.WinColorfulWithWild   -> "Win COLORFUL WILD $name"
            else                                    -> "Noname"
        }
