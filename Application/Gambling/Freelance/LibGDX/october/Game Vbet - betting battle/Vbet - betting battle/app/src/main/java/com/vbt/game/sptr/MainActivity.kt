...
import com.vbt.game.sptr.util.*
import com.vbt.game.sptr.util.manager.DataStoreManager
import com.vbt.game.sptr.util.network.Gist
import com.vbt.game.sptr.util.network.GistData
import com.vbt.game.sptr.util.network.haveNetworkConnection
import com.vbt.game.sptr.webView.webViewFragment
            ...
            coroutineMain.launch(Dispatchers.Main) {
                startFragmentIdFlow.collect { fragmentId ->
                    requestedOrientation = when (fragmentId) {
                        R.id.gameFragment    -> ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
    ...
    private suspend fun getGistData() {
        try {
            Gist.getGistJSON().also { data ->
                GistData.apply {
                    url     = data.getString("ulr")
                    isOffer = data.getString("trub").contains("true")
