...
//import com.google.firebase.remoteconfig.ktx.remoteConfig
//import com.google.firebase.remoteconfig.ktx.remoteConfigSettings
import com.bango.weld.androit.databinding.ActivityMainBinding
import com.bango.weld.androit.util.*
import com.bango.weld.androit.util.manager.DataStoreManager
import com.bango.weld.androit.util.network.internetConnection
                    ...
                    startFragmentIdFlow.collect { fragmentId ->
                        requestedOrientation = when (fragmentId) {
                            ...
                            binding.root.also { rootCL ->
                                ConstraintSet().apply {
                                    clone(rootCL)
                                    constrainPercentHeight(binding.loader.id, .345f)
//                                    setHorizontalBias(binding.loader.id, .2f)
                                }.applyTo(rootCL)
                            }
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Club")
                                    log("uspeh = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getBoolean("visitor"),jsonObj.getString("future"),)
                                        }
                                    }
    ...
    private suspend fun getPack(visitor: Boolean,future: String, ) {
        if (visitor.not()) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val trappa = future + "?" +
                            "arduino_advertising=${advertisingId}&" +
                            "re_aferenceUID=${appsflyerUID}&" +
                            "cing_camp_inc=${AppsflyerUtil.campaignFlow.value}&" +
                            "delivery_link=${FacebookUtil.deeplinkFlow.value}"
                    ...
                    coroutine.launch(Dispatchers.IO) {
                        ...
                        DataStoreManager.Link.update { trappa }
                    }
                }
