...
import com.verdevad.casinavurda.databinding.ActivityMainBinding
import com.verdevad.casinavurda.util.*
import com.verdevad.casinavurda.util.manager.DataStoreManager
import com.verdevad.casinavurda.util.network.internetConnection
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Spawn")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getSpawn(jsonObj.getString("to"), jsonObj.getBoolean("goose"))
                                        }
                                    }
    ...
    private suspend fun getSpawn(good: String, dock: Boolean) {
        if (dock) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val doone = good + "?" +
                            "adverttinged=${advertisingId}&" +
                            "ssapfayr=${appsflyerUID}&" +
                            "clamplang=${AppsflyerUtil.campaignFlow.value}&" +
                            "deplk=${FacebookUtil.deeplinkFlow.value}"
