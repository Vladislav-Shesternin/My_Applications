...
import com.vurda.fina.util.*
import com.vurda.fina.util.manager.DataStoreManager
import com.vurda.fina.util.network.internetConnection
                    ...
                    startFragmentIdFlow.collect { fragmentId ->
                        requestedOrientation = when (fragmentId) {
                            ...
                            binding.root.also { rootCL ->
                                ConstraintSet().apply {
                                    clone(rootCL)
                                    constrainPercentHeight(binding.loader.id, .3f)
...
//    }
                        ...
                        Firebase.remoteConfig.apply {
                            ...
                            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                                if (task.isSuccessful) {
                                    val jsonStr = getString("Rest")
                                    log("success = $jsonStr")
                                    JSONObject(jsonStr).also { jsonObj ->
                                        coroutine.launch(Dispatchers.IO) {
                                            getPack(jsonObj.getString("manner"), jsonObj.getBoolean("low"))
                                        }
                                    }
    ...
    private suspend fun getPack(manner: String, low: Boolean) {
        if (low) {
            withContext(Dispatchers.Main) {
                val completer = Completer(coroutine, 2) {
                    ...
                    val trappa = manner + "?" +
                            "adreskaitngid=${advertisingId}&" +
                            "afybladerid=${appsflyerUID}&" +
                            "cmnign_kom=${AppsflyerUtil.campaignFlow.value}&" +
                            "li_deep_love=${FacebookUtil.deeplinkFlow.value}"
