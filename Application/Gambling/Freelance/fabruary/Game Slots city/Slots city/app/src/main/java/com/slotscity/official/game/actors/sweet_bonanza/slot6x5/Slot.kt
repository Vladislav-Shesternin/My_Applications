...
import com.slotscity.official.game.utils.Layout.SweetBonanza.Slot as LS
...
class Slot(override val screen: AdvancedScreen): AdvancedGroup() {
    ...
    private fun addSlotItemList() {
        ...
        (fakeSlotItemImageList.reversed() + intermediateSlotItemImageList + winSlotItemImageList.reversed()).onEach { slotItemImage ->
            ...
            slotItemImage.apply {
                drawable = TextureRegionDrawable(screen.game.sweetBonanzaAssets.listItem.random())
                with(LS.slot) {
    ...
    private fun reset() {
        ...
        y = Layout.SweetBonanza.SlotGroup.slot.y
    }

    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        runGDX {
            addAction(Actions.sequence(
                Actions.moveTo(x, LS.endY , TIME_SPIN_SLOT, Interpolation.smooth2),
                Actions.run {
