...
//            }
        ...
        onBackPressedDispatcher.addCallback(this) {
            if (views.last().canGoBack()) views.last().goBack()
            else {
                if (views.size > 1) {
                    binding.root.removeView(views.last())
                    views.last().destroy()
                    views.removeLast()
                } else finish()
            }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        views.lastOrNull()?.saveState(outState)
    }

    override fun onResume() {
        super.onResume()
        views.lastOrNull()?.onResume().also {
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        views.lastOrNull()?.restoreState(savedInstanceState)
    }
