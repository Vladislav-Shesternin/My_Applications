...
import com.hepagame.game.manager.MusicManager
...
class LoadingScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(SpriteManager.EnumTexture.PAGRAN.data)
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = msc.apply {
                isLooping = true
                volumeLevelFlow.value = 18.6f
            } }

            stageUI.root.animHide(TIME_ANIM) {
                game.activity.lottie.hide()

                val screenName = if (game.gamePrefsDialog.contains("lol")) MenuScreen::class.java.name else DianisScreen::class.java.name
