...
import com.dogbytegames.offtheroa.game.manager.MusicManager
...
class LoadingScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(SpriteManager.EnumTexture.LDR.data)
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = gold.apply {
                isLooping = true
            ...
            stageUI.root.animHide(TIME_ANIM) {
                ...
                val screenName = if (game.prefsDialog.contains("dupa")) MenuScreen::class.java.name else DupaPrivet::class.java.name
