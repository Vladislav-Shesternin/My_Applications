package com.bigag.farm.garden.village.offline.farming.game.game.box2d
...
import com.bigag.farm.garden.village.offline.farming.game.game.utils.SizeStandardizer
import com.bigag.farm.garden.village.offline.farming.game.game.utils.actor.setBounds
...
import com.bigag.farm.garden.village.offline.farming.game.game.utils.toB2
import com.bigag.farm.garden.village.offline.farming.game.game.utils.toUI
import com.bigag.farm.garden.village.offline.farming.game.util.cancelCoroutinesAll
...
abstract class AbstractBodyGroup: Destroyable {
    ...
    private val _actorList = mutableSetOf<com.bigag.farm.garden.village.offline.farming.game.game.utils.advanced.AdvancedGroup>()
    ...
    protected val colorJoint = Color.ORANGE.cpy()
    ...
    protected fun Vector2.subCenterBG(body: Body): Vector2 = toStandartBG.toB2.sub((body.userData as AbstractBody).center)

    protected fun com.bigag.farm.garden.village.offline.farming.game.game.utils.advanced.AdvancedGroup.setBoundsStandartBG(x: Float, y: Float, width: Float, height: Float) {
    ...
    protected fun drawJoint(
        ...
        screenBox2d.drawerUtil.drawer.line(
            tmpPositionA.set(bodyA.body?.position).add(tmpAnchorA.set(anchorA).subCenterBG(bodyA.body!!)).toUI,
            tmpPositionB.set(bodyB.body?.position).add(tmpAnchorB.set(anchorB).subCenterBG(bodyB.body!!)).toUI,
            colorJoint.apply { a = alpha }, 1f
        )
    }

    fun setNoneId() {
        _bodyList.onEach { it.setNoneId() }
    }

    fun setOriginalId() {
        _bodyList.onEach { it.setOriginalId() }
    }
