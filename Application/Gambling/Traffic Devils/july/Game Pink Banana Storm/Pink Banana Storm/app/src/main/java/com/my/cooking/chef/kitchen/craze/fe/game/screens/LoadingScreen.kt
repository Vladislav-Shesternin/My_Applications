...
import com.my.cooking.chef.kitchen.craze.fe.game.manager.MusicManager
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(
                SpriteManager.EnumTexture.b1.data,
                SpriteManager.EnumTexture.b2.data,
                SpriteManager.EnumTexture.b3.data,
            )
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = organ.apply {
                isLooping = true
                volumeLevelFlow.value = 30f
            } }

            stageUI.root.animHide(TIME_ANIM) {
                game.activity.lottie.hide()
                val screenName = if (game.prefsDialog.contains("OSTAP")) MenuScreen::class.java.name else Dilegatsia::class.java.name
