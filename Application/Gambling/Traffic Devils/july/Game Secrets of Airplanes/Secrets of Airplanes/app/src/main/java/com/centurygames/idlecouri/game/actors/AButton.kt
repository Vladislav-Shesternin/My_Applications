package com.centurygames.idlecouri.game.actors
...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Back -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.alpha.bbb),
            pressed = TextureRegionDrawable(screen.game.alpha.ddd),
            disabled = TextureRegionDrawable(screen.game.alpha.ddd),
        )
    }
