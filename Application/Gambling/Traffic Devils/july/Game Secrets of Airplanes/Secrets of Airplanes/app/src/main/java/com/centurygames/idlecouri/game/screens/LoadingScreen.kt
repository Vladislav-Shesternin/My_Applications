...
import com.centurygames.idlecouri.game.manager.MusicManager
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(
                SpriteManager.EnumTexture.fofA.data,
                SpriteManager.EnumTexture.fofB.data,
            )
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = fly.apply {
                isLooping = true
            ...
            stageUI.root.animHide(TIME_ANIM) {
                ...
                val screenName = if (game.prefsDialog.contains("PERliNA")) MenuScreen::class.java.name else WelcomeScreen::class.java.name
