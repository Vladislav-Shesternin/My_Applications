package com.bitmango.go.game.actors
...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Back -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.bbb.bavg),
            pressed = TextureRegionDrawable(screen.game.bbb.yytweh),
            disabled = TextureRegionDrawable(screen.game.bbb.yytweh),
        )
    }
