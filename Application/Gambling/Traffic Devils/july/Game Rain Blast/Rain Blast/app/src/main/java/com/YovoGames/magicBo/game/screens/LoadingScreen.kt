...
import com.YovoGames.magicBo.game.manager.MusicManager
...
class LoadingScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(SpriteManager.EnumTexture.Soloha.data)
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = sweet.apply {
                isLooping = true
            ...
            stageUI.root.animHide(TIME_ANIM) {
                ...
                val screenName = if (game.prefsDialog.contains("ooo(AsidaSdj")) MenuScreen::class.java.name else Rom::class.java.name
