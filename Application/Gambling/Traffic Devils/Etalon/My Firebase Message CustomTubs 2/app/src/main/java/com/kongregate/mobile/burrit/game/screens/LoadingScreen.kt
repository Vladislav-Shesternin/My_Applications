...
import com.kongregate.mobile.burrit.game.manager.MusicManager
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTexturesList = mutableListOf(
                SpriteManager.EnumTexture.bacMini.data,
            )
            loadTexture()
        }
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = musicaa.apply {
                isLooping = true
            ...
            stageUI.root.animHide(TIME_ANIM) {
                ...
                val screenName = if (game.prefsDialog.contains("popol_papel")) MenuScreen::class.java.name else GeraltScreen::class.java.name
