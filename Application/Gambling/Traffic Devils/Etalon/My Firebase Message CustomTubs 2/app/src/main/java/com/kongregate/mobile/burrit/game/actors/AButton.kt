package com.kongregate.mobile.burrit.game.actors
...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Back -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.alpha.ba),
            pressed = TextureRegionDrawable(screen.game.alpha.backl),
            disabled = TextureRegionDrawable(screen.game.alpha.backl),
        )
    }
