...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Back -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.aALL.gdef),
            pressed = TextureRegionDrawable(screen.game.aALL.gpers),
            disabled = TextureRegionDrawable(screen.game.aALL.gpers),
        )
    }
