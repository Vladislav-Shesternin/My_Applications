...
import com.amayasoft.cars.kids.toddlers.garage.ga.game.manager.MusicManager
...
//                    if (progress % 85 == 0) log("progress = $progress%")
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply { music = fruit.apply {
                isLooping = true
                volumeLevelFlow.value = 16f
            } }

            stageUI.root.animHide(TIME_ANIM) {
                game.activity.lottie.hide()
                val screenName = if (game.prefsDialog.contains(fiesta)) MenuScreen::class.java.name else Derulo::class.java.name
