package com.minimuffin.gardenstor.game.actors.button
...
open class AButton(
    ...
    private fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            screen.game.soundUtil.apply { play(TOUCH_DOWN) }
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.mEnU -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.assets.mdef),
            pressed = TextureRegionDrawable(screen.game.assets.mpres),
            disabled = TextureRegionDrawable(screen.game.assets.mpres),
        )
    }
