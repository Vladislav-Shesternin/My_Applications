...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Back -> Static.AButtonStyle(
            default = TextureRegionDrawable(screen.game.assets.b_def),
            pressed = TextureRegionDrawable(screen.game.assets.b_press),
            disabled = TextureRegionDrawable(screen.game.assets.b_press),
        )
    }
