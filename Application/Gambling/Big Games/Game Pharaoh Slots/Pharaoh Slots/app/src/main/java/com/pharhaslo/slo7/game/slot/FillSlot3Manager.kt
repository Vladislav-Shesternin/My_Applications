package com.pharhaslo.slo7.game.slot

import com.pharhaslo.slo7.game.actors.slot.*
import com.pharhaslo.slo7.game.assets.SpriteManager
import com.pharhaslo.slo7.game.assets.SpriteManager.SlotItemSpriteList.*
import com.pharhaslo.slo7.game.utils.log
import kotlin.random.Random

lateinit var slotItemAll  : SlotItem
lateinit var slotItemBonus: SlotItem

// 9 - элементов необходимо для текущего алгоритма заполнения
val megaslotItemList get() = listOf<SlotItem>(
...
class FillSlot3Manager(
    ...
    init {
        slotItemAll   = SlotItem(FillSlot3Manager.SLOT_ITEM_ALL_ID, 3f, SpriteManager.GameSprite.SLOT_ITEM_ALL.data.texture  )
        slotItemBonus = SlotItem(FillSlot3Manager.SLOT_ITEM_BONUS_ID, 0f, SpriteManager.GameSprite.SLOT_ITEM_BONUS.data.texture)
    ...
    private fun fillMix(isUseSlotItemAll: Boolean = true, isUseSlotItemBonus: Boolean = true) {
        ...
        val by3SlotItemListList = megaslotItemList.shuffled().chunked(3)
    ...
    private fun fillWin(isUseSlotItemAll: Boolean = false) {
        ...
        combinationList.onEach { combination ->
            val winSlotItem = megaslotItemList.random()
