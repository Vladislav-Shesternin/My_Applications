...
import com.pharhaslo.slo7.activityContext
...
object DataStoreManager {
    ...
    suspend fun collectBalance(block: suspend (Long) -> Unit) {
        activityContext.dataStore.data.collect { block(it[BALANCE_KEY] ?: 20_000L) }
    }

    suspend fun updateBalance(block: suspend (Long) -> Long) {
        activityContext.dataStore.edit { it[BALANCE_KEY] = block(it[BALANCE_KEY] ?: 20_000L) }
    }
