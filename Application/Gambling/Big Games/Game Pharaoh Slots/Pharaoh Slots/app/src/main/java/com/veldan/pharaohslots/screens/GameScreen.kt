...
import com.veldan.pharaohslots.actors.slot.Bonus
import com.veldan.pharaohslots.actors.slot.GoResult
import com.veldan.pharaohslots.actors.slot.SlotGroup
import com.veldan.pharaohslots.actors.slot.SuperGameGroup
...
import com.veldan.pharaohslots.assets.FontTTFManager
...
import com.veldan.pharaohslots.manager.DataStoreManager
...
import com.veldan.pharaohslots.utils.SlotGroup as SG
import com.veldan.pharaohslots.advanced.AdvancedGroup.AlignmentHorizontal as AH
import com.veldan.pharaohslots.advanced.AdvancedGroup.AlignmentVertical as AV
...
class GameScreen : AdvancedScreen() {
    ...
    private fun AdvancedStage.addBetPanel() {
        val group = AdvancedGroup().apply {
            ...
            val imagePanel = Image(SpriteManager.GameSprite.BET_PANEL.data.texture)
    ...
    private fun AdvancedStage.addFreeGoCount() {
        val label = Label("", Label.LabelStyle(FontTTFManager.EnumFont.SIGMAR_ONE_53.data.font, Color.WHITE)).apply {
            ...
            coroutineFreeGo.launch { freeGoFlow.collect { count -> Gdx.app.postRunnable {
                setText(count)
                if (count > 0) listOf(freeGoImage, this@apply).onEach { it.addAction(Actions.fadeIn(TIME_SHOW_FREE_GO)) }
                else listOf(freeGoImage, this@apply).onEach { it.addAction(Actions.fadeOut(TIME_SHOW_FREE_GO)) }
            } } }
        }
        addActor(label)
    }
                ...
                when (state) {
                    AutogoState.GO -> {
                        ...
                        CoroutineScope(Dispatchers.Default).launch {
                            ...
                            if (autogoStateFlow.value == AutogoState.GO) autogoStateFlow.value = AutogoState.DEFAULT
