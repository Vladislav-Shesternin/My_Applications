package com.srata.financialguru.game.utils.actor
...
import com.srata.financialguru.game.utils.Size
...
suspend fun Actor.animSuspendVisible(time: Float = 0f) = CompletableDeferred<Unit>().also { continuation ->
    runGDX {
...
fun Actor.animInvisible(time: Float = 0f, block: () -> Unit = {}) {
    addAction(Actions.sequence(
        Actions.fadeOut(time),
        Actions.run { block() }
    ))
