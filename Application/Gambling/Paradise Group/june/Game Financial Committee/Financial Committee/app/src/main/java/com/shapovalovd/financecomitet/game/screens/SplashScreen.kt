...
import com.shapovalovd.financecomitet.game.manager.*
...
class SplashScreen : AdvancedScreen() {
   ...
   // private val paragrasssssa by lazy { Label("", Label.LabelStyle(FontTTFManager.RegularraFont.font_100.font, Color.WHITE)) }
    ...
    private fun loadAssets() {
        ...
        with(FontTTFManager) {
            loadableListFont = (FontTTFManager.NunitoSans.values + FontTTFManager.PopMedium.values + FontTTFManager.PopSemiBold.values).toMutableList()
        ...
        if (isFinishProgress && isFinishAnim) {
            ...
            coroutine.launch(Dispatchers.IO) {
                ...
                runGDX {
                    ...
                    NavigationManager.navigate(if (isAgree) StartScreen() else ParadocScreen())
                }
            }
        }
    }
