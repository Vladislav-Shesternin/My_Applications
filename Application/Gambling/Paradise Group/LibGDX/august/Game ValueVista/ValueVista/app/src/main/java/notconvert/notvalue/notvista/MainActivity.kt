...
import notconvert.notvalue.notvista.databinding.ActivityMainBinding
import notconvert.notvalue.notvista.util.AppsflyerUtil
import notconvert.notvalue.notvista.util.Lottie
import notconvert.notvalue.notvista.util.Once
import notconvert.notvalue.notvista.util.internetConnection
import notconvert.notvalue.notvista.util.log
import notconvert.notvalue.notvista.util.manager.DataStoreManager
import notconvert.notvalue.notvista.util.webView.WebViewFragment
...
class MainActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {
                    ...
                    startFragmentIdFlow.collect { fragmentId ->
                        when (fragmentId) {
                            R.id.libGDXFragment                                             -> {
                                ...
                                setNavigationBarColor(notconvert.notvalue.notvista.R.color.black)
                                setStartDestination(fragmentId)
                                ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
                            }
                            notconvert.notvalue.notvista.MainActivity.Companion.WEB_VIEW_ID -> {
    ...
    private fun initialize() {
        ...
        navController = findNavController(notconvert.notvalue.notvista.R.id.nav_host_fragment)
    ...
    private fun setStartDestination(@IdRes destinationId: Int) {
        navController.run { navInflater.inflate(notconvert.notvalue.notvista.R.navigation.nav_graph).apply { setStartDestination(destinationId) }.also { setGraph(it, null) } }
    }
        ...
        try {
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    notconvert.notvalue.notvista.MainActivity.Companion.startFragmentIdFlow.tryEmit(R.id.libGDXFragment)
                }
            }
        } catch (e: IOException) {
            ...
            when (DataStoreManager.Key.get()) {
                "SUCCESS" -> {
                    DataStoreManager.Link.get()?.let {
                        ...
                        notconvert.notvalue.notvista.MainActivity.Companion.webURL = it
                        notconvert.notvalue.notvista.MainActivity.Companion.startFragmentIdFlow.emit(
                            notconvert.notvalue.notvista.MainActivity.Companion.WEB_VIEW_ID
                        )
                    }
                }
        ...
        Firebase.remoteConfig.apply {
            ...
            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                if (task.isSuccessful) {
                    val jsonData = getString("ValueVistaData")
                    log("success = $jsonData")
                    JSONObject(jsonData).also { jsonObj ->
                        coroutine.launch(Dispatchers.IO) {
                            buildSsilkaAndOpenWeb(
                                jsonObj.getString("dev_keyalarmo_tiktonik"),
                                jsonObj.getString("paradontaks_pofeerr"),
                            )
                        }
                    }
                } else {
    ...
    private fun buildSsilkaAndOpenWeb(devKey: String, ssilka: String) {
        coroutine.launch {
            ...
            completerFlow.collect { if (++counter == 1) {
                val fullSsilka = ssilka.plus("?") +
                        ...
                        "sub_id_8=" .plus(AppsFlyerLib.getInstance().getAppsFlyerUID(notconvert.notvalue.notvista.appContext)).plus("&") +
                        ...
                        "sub_id_12=".plus(notconvert.notvalue.notvista.appContext.packageName)

                log("full: $fullSsilka")

                notconvert.notvalue.notvista.MainActivity.Companion.webURL = fullSsilka
                notconvert.notvalue.notvista.MainActivity.Companion.startFragmentIdFlow.tryEmit(notconvert.notvalue.notvista.MainActivity.Companion.WEB_VIEW_ID)
