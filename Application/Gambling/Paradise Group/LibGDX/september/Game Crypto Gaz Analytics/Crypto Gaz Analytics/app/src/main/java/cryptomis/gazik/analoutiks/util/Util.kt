package cryptomis.gazik.analoutiks.util
...
abstract class AbstractDataStore {
    ...
    abstract inner class DataStoreElement<T> {
        ...
        open suspend fun collect(block: suspend (T?) -> Unit) {
            cryptomis.gazik.analoutiks.appContext.dataStore.data.collect { block(it[key]) }
        }

        open suspend fun update(block: suspend (T?) -> T) {
            cryptomis.gazik.analoutiks.appContext.dataStore.edit { it[key] = block(it[key]) }
        }

        open suspend fun get(): T? {
            return cryptomis.gazik.analoutiks.appContext.dataStore.data.first()[key]
        }
    }
}