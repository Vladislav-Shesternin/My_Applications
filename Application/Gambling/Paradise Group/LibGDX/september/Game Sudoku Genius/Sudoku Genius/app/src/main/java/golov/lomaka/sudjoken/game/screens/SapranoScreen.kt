...
class SapranoScreen(override val game: LibGDXGame) : AdvancedScreen() {
...
//    }
    ...
    private fun loadAssets() {
        ...
        with(game.fontTTFManager) {
            loadableFontList = (game.fontTTFUtil.fontInterRegular.values).toMutableList()
            load()
        }
...
//        }
    ...
    private fun isFinish() {
        if (isFinishProgress && isFinishAnim) {
            ...
            game.musicUtil.apply { music = RELAX.apply { isLooping = true } }

            game.activity.lottie.hideLoader()
            game.navigationManager.navigate(CommongScreen(game))
        }
    }
