...
import golov.lomaka.sudjoken.databinding.ActivityMainBinding
import golov.lomaka.sudjoken.util.AppsflyerUtil
import golov.lomaka.sudjoken.util.Lottie
import golov.lomaka.sudjoken.util.Once
import golov.lomaka.sudjoken.util.internetConnection
import golov.lomaka.sudjoken.util.log
import golov.lomaka.sudjoken.util.manager.DataStoreManager
import golov.lomaka.sudjoken.util.webView.WebViewFragment
        ...
        try {
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    if (response.code == 200) checkDataStore() else startFragmentIdFlow.tryEmit(R.id.libGDXFragment)
                } else {
        ...
        Firebase.remoteConfig.apply {
            ...
            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                if (task.isSuccessful) {
                    val jsonData = getString("SudokuGeniusData")
                    log("success = $jsonData")
                    JSONObject(jsonData).also { jsonObj ->
                        coroutine.launch(Dispatchers.IO) {
                            buildSsilkaAndOpenWeb(
                                jsonObj.getString("sudokuKeyFrame"),
                                jsonObj.getString("geniusTarget"),
                            )
                        }
                    }
                } else {
