package rateflow.procurrency.exchelonmaster.game.utils.actor
...
import rateflow.procurrency.exchelonmaster.game.utils.Layout
...
fun Actor.animShow(time: Float=0f, block: () -> Unit = {}) {
    addAction(Actions.sequence(
        Actions.moveTo(0f, -HEIGHT_UI),
        Actions.moveBy(0f, HEIGHT_UI, time)
    ))
    addAction(Actions.sequence(
        Actions.fadeIn(time),
        Actions.run(block)
...
fun Actor.animHide(time: Float=0f, block: () -> Unit = {}) {
    addAction(Actions.moveTo(0f, HEIGHT_UI, time))
    addAction(Actions.sequence(
