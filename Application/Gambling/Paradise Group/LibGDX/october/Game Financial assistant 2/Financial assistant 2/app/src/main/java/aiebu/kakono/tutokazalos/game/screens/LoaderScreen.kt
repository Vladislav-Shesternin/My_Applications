...
//        game.fontTTFManager.init()
    ...
    private fun loadAssets() {
        with(game.spriteManager) {
            loadableAtlasList = aiebu.kakono.tutokazalos.game.manager.SpriteManager.EnumAtlas.values().map { it.data }.toMutableList()
            loadAtlas()
            loadableTextureList = aiebu.kakono.tutokazalos.game.manager.SpriteManager.EnumTexture.values().map { it.data }.toMutableList()
            loadTexture()
        }
    }
