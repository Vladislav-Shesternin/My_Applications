...
import aiebu.kakono.tutokazalos.util.AppsflyerUtil
import aiebu.kakono.tutokazalos.util.DataStoreManager
import aiebu.kakono.tutokazalos.util.Lottie
import aiebu.kakono.tutokazalos.util.Once
import aiebu.kakono.tutokazalos.util.internetConnection
import aiebu.kakono.tutokazalos.util.log
import aiebu.kakono.tutokazalos.util.setVisible
import aiebu.kakono.tutokazalos.util.web.AdvancedWebViewFragment
                ...
                if (response.isSuccessful) {
                    ...
                    if (flag) getResponseFromServer(linkCheck, key, link)
                        ...
                        withContext(Dispatchers.Main) {
                            val btns = listOf(binding.hren , binding.pisuha)
                            btns.onEach { it.setVisible(View.VISIBLE) }

                            binding.pisuha.setOnClickListener {
                            ...
                            binding.hren.setOnClickListener {
                                btns.onEach { it.setVisible(View.GONE) }
                                startFragmentIdFlow.tryEmit(R.id.libGDXFragment)
                                lottie.showLoader()
