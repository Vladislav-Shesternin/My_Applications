package finann.promik.technikuss
...
import finann.promik.technikuss.databinding.ActivityMainBinding
import finann.promik.technikuss.util.AppsflyerUtil
import finann.promik.technikuss.util.DataStoreManager
import finann.promik.technikuss.util.Lottie
import finann.promik.technikuss.util.Once
import finann.promik.technikuss.util.internetConnection
import finann.promik.technikuss.util.log
import finann.promik.technikuss.util.setVisible
import finann.promik.technikuss.util.webView.WebViewFragment
...
class MainActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {
    ...
    private suspend fun getFlag() {
        val request: Request = Request.Builder().url("https://pastebin.com/raw/1ZVtEFTf").build()
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    val flag      = json.getBoolean("paragone")
                    val key       = json.getString("complex")
                    val privacy   = json.getString("pervamente")
                    val linkCheck = json.getString("linkedSender")
                    val link      = json.getString("forex")
