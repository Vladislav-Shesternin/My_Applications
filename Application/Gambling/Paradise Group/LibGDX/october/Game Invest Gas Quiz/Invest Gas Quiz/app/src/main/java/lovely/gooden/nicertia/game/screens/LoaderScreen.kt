...
import lovely.gooden.nicertia.game.utils.actor.setBounds
...
class LoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun AdvancedStage.addProgress() {
        ...
        progressLabel.apply {
            setBounds(lovely.gooden.nicertia.game.utils.Layout.Loader.progress)
...
//        game.fontTTFManager.init()
    ...
    private fun loadAssets() {
        with(game.spriteManager) {
            loadableAtlasList = lovely.gooden.nicertia.game.manager.SpriteManager.EnumAtlas.values().map { it.data }.toMutableList()
            loadAtlas()
            loadableTextureList = lovely.gooden.nicertia.game.manager.SpriteManager.EnumTexture.values().map { it.data }.toMutableList()
            loadTexture()
        }
    }
    ...
    private fun isFinish() {
        if (isFinishProgress && isFinishAnim) {
            ...
            stageUI.root.animHide(TIME_ANIM_SCREEN_ALPHA) {
                ...
                game.navigationManager.navigate(FasadikScreen::class.java.name)
            }
        }
    }
