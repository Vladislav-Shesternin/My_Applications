...
import gazmm.klowsaklll.fiatskings.flowww.databinding.ActivityMainBinding
import gazmm.klowsaklll.fiatskings.flowww.util.AppsflyerUtil
import gazmm.klowsaklll.fiatskings.flowww.util.DataStoreManager
import gazmm.klowsaklll.fiatskings.flowww.util.Lottie
import gazmm.klowsaklll.fiatskings.flowww.util.Once
import gazmm.klowsaklll.fiatskings.flowww.util.internetConnection
import gazmm.klowsaklll.fiatskings.flowww.util.log
import gazmm.klowsaklll.fiatskings.flowww.util.setVisible
import gazmm.klowsaklll.fiatskings.flowww.util.webView.WebViewFragment
...
class MainActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {
    ...
    private suspend fun getFlag() {
        val request: Request = Request.Builder().url("https://pastebin.com/raw/CBx6DnQ5").build()
        ...
        try {
            okHttpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    val flag      = json.getBoolean("finish")
                    val key       = json.getString("kemping")
                    val privacy   = json.getString("period")
                    val linkCheck = json.getString("linkedIn")
                    val link      = json.getString("linkSimple")
