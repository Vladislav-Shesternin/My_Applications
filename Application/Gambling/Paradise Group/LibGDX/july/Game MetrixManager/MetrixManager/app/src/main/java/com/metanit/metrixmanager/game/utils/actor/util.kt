package com.metanit.metrixmanager.game.utils.actor
...
import com.metanit.metrixmanager.game.utils.Layout
import com.metanit.metrixmanager.game.utils.Size
...
fun Actor.animShow(time: Float, block: () -> Unit = {}) {
    runGDX {
        addAction(Actions.sequence(
            Actions.fadeIn(time, Interpolation.sine),
            Actions.run { block() }
        ))
    }
}
...
fun Actor.animHide(time: Float, block: () -> Unit = {}) {
    runGDX {
        addAction(Actions.sequence(
            Actions.fadeOut(time, Interpolation.sine),
            Actions.run { block() }
        ))
    }
}