package com.lakalutic.statisticsmanager
...
import com.lakalutic.statisticsmanager.util.*
import com.lakalutic.statisticsmanager.util.manager.DataStoreManager
import com.lakalutic.statisticsmanager.util.network.internetConnection
import com.lakalutic.statisticsmanager.webView.WebViewFragment
...
class LambarginiActivity : AppCompatActivity(), AndroidFragmentApplication.Callbacks {

    companion object {
        const val WEB_VIEW_ID = 1000

        @SuppressLint("StaticFieldLeak") lateinit var navController: NavController
        lateinit var binding: PilotkaBinding
    ...
    private fun initialize() {
        binding = PilotkaBinding.inflate(layoutInflater)
    ...
    private fun setNavigationBarColor(@ColorRes colorId: Int) {
        coroutine.launch(Dispatchers.Main) { window.navigationBarColor = ContextCompat.getColor(this@LambarginiActivity, colorId) }
    }
