package com.lakalutic.statisticsmanager.webView
...
class WebViewFragment(
    private val activity: LambarginiActivity
) {
    ...
    fun onCreate(coroutine: CoroutineScope) {
        onBackPressed()

        this.coroutine = coroutine
        webView = LambarginiActivity.binding.megaView.apply {
            with(settings) {
                savePassword = true
                saveFormData = true
                mixedContentMode = 0
    ...
    fun showAndOpenUrl() {
        ...
        coroutine?.launch(Dispatchers.Main) {
            ...
            webView?.loadUrl(LambarginiActivity.webViewURL)
