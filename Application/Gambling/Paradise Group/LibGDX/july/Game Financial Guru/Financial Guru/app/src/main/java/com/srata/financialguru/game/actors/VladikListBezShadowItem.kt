...
class VladikListBezShadowItem(
    ...
    private val cudaLbl = Label("${numStr(10, 90, 1)},000", Label.LabelStyle(FontTTFManager.GilMed.font_28.font, GameColor.bbb))


    override fun sizeChanged() {
        if (width > 0 && height > 0) {
            addAndFillActor(Image(SpriteManager.GameRegion.BEZ_TEN.region))
            addActors(logoImg, titlLbl, cudaLbl)
            logoImg.setBounds(24f, 31f, 76f, 76f)
            titlLbl.setBounds(122f, 68f, 229f, 34f)
            cudaLbl.setBounds(121f, 34f, 172f, 34f)
