package com.nicoledeonnit.cryptosignals.game.utils.actor
...
import com.nicoledeonnit.cryptosignals.game.utils.Layout
import com.nicoledeonnit.cryptosignals.game.utils.Size
...
fun Actor.animpokazat(time: Float, block: () -> Unit = {}) {
    runGDX {
        addAction(Actions.sequence(
            Actions.fadeIn(time, Interpolation.pow3In),
            Actions.run { block() }
        ))
    }
}
...
fun Actor.animHide(time: Float, block: () -> Unit = {}) {
    runGDX {
        addAction(Actions.sequence(
            Actions.fadeOut(time, Interpolation.pow3In),
            Actions.run { block() }
        ))
    }
}