...
import com.shapovalovd.financecomitet.databinding.ActivityMainBinding
import com.shapovalovd.financecomitet.util.*
import com.shapovalovd.financecomitet.util.network.internetConnection
import com.shapovalovd.financecomitet.webView.webViewFragment
import com.shapovalovd.financecomitet.util.manager.DataStoreManager
import com.shapovalovd.financecomitet.util.network.NetworkUtil
        ...
        Firebase.remoteConfig.apply {
            ...
            fetchAndActivate().addOnCompleteListener(this@MainActivity) { task ->
                if (task.isSuccessful) {
                    val jsonData = getString("jsonDatasdd")
                ...
                withContext(Dispatchers.Main) {
                    val completer = Completer(coroutine, 1) {
                        ...
                        val fullUrl = offer
                            ...
                            .replace(
                                "{cost_usd}",
                                AppsflyerUtil.dataMap["cost_cents_USD"].toString()
                            )
                            .replace(
                                "{media_source}",
                                AppsflyerUtil.dataMap["media_source"].toString()
                            )
