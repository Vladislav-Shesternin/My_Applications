...
import com.gusarove.digitalexchange.MainActivity
import com.gusarove.digitalexchange.game.game
import com.gusarove.digitalexchange.game.manager.*
...
class SplashScreen : AdvancedScreen() {
   ...
   // private val paragrasssssa by lazy { Label("", Label.LabelStyle(FontTTFManager.RegularraFont.font_100.font, Color.WHITE)) }
    ...
    private fun AdvancedGroup.addKoshel() {
        val image = Image(SpriteManager.SplashRegion.ESHLE.region)
        addActor(image)
        image.setBounds(84f, 650f, 721f, 954f)
    }
    ...
    private fun loadSplashAssets() {
        with(SpriteManager) {
            loadableTextureList = mutableListOf(SpriteManager.EnumTexture.GARAG, SpriteManager.EnumTexture.ESHLE)
    ...
    private fun loadAssets() {
        ...
        with(FontTTFManager) {
            loadableListFont = (FontTTFManager.LeilaBold.values + FontTTFManager.LeilaRegular.values + FontTTFManager.Roboto.values).toMutableList()
        ...
        coroutine.launch {
            ...
            progressFlow.collect { p ->
                while (progress < (p * 100)) {
                    ...
                    if (progress % 2 == 0) log("progress = $progress%")
        ...
        if (isFinishProgress && isFinishAnim) {
            ...
            coroutine.launch(Dispatchers.IO) {
                ...
                runGDX {
                    ...
                    NavigationManager.navigate(if (isAgree) GoodNightScreen() else PiriviciScreen())
                }
            }
        }
    }
