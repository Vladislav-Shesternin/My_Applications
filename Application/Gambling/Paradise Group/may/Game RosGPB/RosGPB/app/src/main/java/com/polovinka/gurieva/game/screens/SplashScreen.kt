...
import com.polovinka.gurieva.game.manager.*
...
class SplashScreen : AdvancedScreen() {
    ...
    private val paragrasssssa by lazy { Label("", Label.LabelStyle(FontTTFManager.RegFont.font_100.font, Color.WHITE)) }
    ...
    private fun loadSplashAssets() {
        with(SpriteManager) {
            loadableTextureList = mutableListOf(SpriteManager.EnumTexture.BASGAR)
            loadTexture(game.assetManager)
        }
        with(FontTTFManager) {
            loadableListFont = (FontTTFManager.RegFont.values).toMutableList()
        ...
        if (isFinishProgress && isFinishAnim) {
            ...
            coroutine.launch(Dispatchers.IO) {
                ...
                runGDX {
                    ...
                    NavigationManager.navigate(if (isAgree) CardScreen() else PicivyScreen())
                }
            }
        }
    }
