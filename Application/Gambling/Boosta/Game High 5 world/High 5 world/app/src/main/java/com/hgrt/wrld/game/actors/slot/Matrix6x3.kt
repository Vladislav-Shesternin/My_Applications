...
class Matrix6x3(
    ...
    private val slotF = listOf<Item>(f1, f2, f3)
    ...
    fun generateSlot(slotIndex: Int): List<SlotItem> = mutableListOf<SlotItem>().apply {
        if (this@Matrix6x3::shuffledSlotItemList.isInitialized.not()) throw Exception("Используй метод initialize")
