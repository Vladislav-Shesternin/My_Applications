...
import com.wlfe.astiener.game.util.Layout.Slot as LS

class Slot: AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 3
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 27

        const val TIME_SPIN_SLOT = 1.4f
    }
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        addAction(Actions.sequence(
            Actions.moveTo(x, LS.endY, TIME_SPIN_SLOT, Interpolation.swing),
            Actions.run {
                reset()
