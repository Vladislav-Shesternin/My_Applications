...
import com.rcks.scaloi.game.util.Layout.Slot as LS

class Slot: AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 5
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 90
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        addAction(Actions.sequence(
            Actions.moveTo(x, LS.endY, TIME_SPIN_SLOT, Interpolation.pow2Out),
            Actions.run {
                reset()
