...
class GalaxyLoaderScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private fun loadSplashAssets() {
        with(game.spriteManager) {
            loadableTextureList = mutableListOf(SpriteManager.EnumTexture.FONIK.data)
            loadTexture()
        }
    ...
    private fun isFinish() {
        if (isFinishProgress) {
            ...
            game.musicUtil.apply {
                coff  = 0.3f
                music = KIM.apply { isLooping = true }
            }

            stageUI.root.animHide(TIME_ANIM) {
                game.navigationManager.navigate(GalaxyGameScreen::class.java.name)
            }
        }
    }
