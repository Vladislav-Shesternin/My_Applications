...
import com.egyptian.rebirth.gremmy.util.Layout.Slot as LS

class Slot: AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 4
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 60
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        addAction(Actions.sequence(
            Actions.moveTo(x, LS.endY, TIME_SPIN_SLOT, Interpolation.circleIn),
            Actions.run {
                reset()
