...
import com.aztec.blaze.game.util.Layout.Slot as LS

class Slot: AdvancedGroup() {

    companion object {
        const val SLOT_ITEM_VISIBLE_COUNT      = 5
        const val SLOT_ITEM_INTERMEDIATE_COUNT = 10

        const val TIME_SPIN_SLOT = 1f
    }
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        addAction(Actions.sequence(
            Actions.moveTo(x, LS.endY, TIME_SPIN_SLOT),
            Actions.run {
                reset()
