...
class Matrix5x5(
    ...
    private val slotD = listOf<Item>(d1, d2, d3, d4, d5)
    private val slotE = listOf<Item>(e1, e2, e3, e4, e5)
    ...
    fun generateSlot(slotIndex: Int): List<SlotItem> = mutableListOf<SlotItem>().apply {
        if (this@Matrix5x5::shuffledSlotItemList.isInitialized.not()) throw Exception("Используй метод initialize")
