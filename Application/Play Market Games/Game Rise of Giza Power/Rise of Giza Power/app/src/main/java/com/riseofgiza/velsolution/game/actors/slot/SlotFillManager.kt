...
import com.riseofgiza.velsolution.util.probability
...
class SlotFillManager(val slotList: List<Slot>) {
    ...
    private fun fillMix(isUseWild: Boolean = true) {
        ...
        val combinationMatrix3x3: CombinationMatrix3x3 = if (isUseWild && probability(20)) {
            log("fillMix use wild")
            Combination3x3.MixWithWild
        } else Combination3x3.Mix
    ...
    private fun fillWin() {
        ...
        val combinationMatrix3x3 = listOf<CombinationMatrix3x3>(
            ...
            Combination3x3.WinColorful,
            Combination3x3.WinColorfulWithWild,
        ).shuffled().first()
